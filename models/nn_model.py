...
class NeuralNetworkModel(ModelTrainer):
    ...
    @staticmethod
    def build_model(input_size, hidden_size, lr, class_weights):
    ...
    def train(self, x, y):
        ...
        for lr in self.param_grid['lr']:
            wrapper = self.build_model(input_size, 3, lr, class_weights=class_weights)
            for epoch in range(self.param_grid['epochs']):
                ...
                if (epoch + 1) % 40 == 0 or epoch == self.param_grid['epochs'] - 1:
                    print(f'Epoch [{epoch + 1}/{self.param_grid["epochs"]}], LR: {lr}, Accuracy: {val_accuracy}')

                if val_accuracy > best_score:
                    best_score = val_accuracy
                    self.model = copy.deepcopy(wrapper)
